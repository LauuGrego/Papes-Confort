 ...
 * resolving any double-encoded UTF-8 sequences, replacement characters (\uFFFD), and corrupted tildes/eñes.
 */
...
export function cleanGescomText(text: string | number | null | undefined): string | undefined {
  ...
  // 1. Fix double-encoded UTF-8 sequences (e.g. "automÃ¡tico" -> "automático")
  if (/[\u00C2-\u00C5][\u0080-\u00BF]/.test(str)) {
    try {
  ...
  // 2. Fix corrupted Spanish replacement characters (\uFFFD)
  if (str.includes('\uFFFD')) {
